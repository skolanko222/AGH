...
std::fstream plik ,plik_m;

//g++ main.cpp -lgsl -lgslcblas -lm && ./a.out
double fun(double x,int nr);
gsl_vector *druga_pochodna(int nr, gsl_vector *x);

int main(void)
{
	plik.open("out.txt");
	plik_m.open("pochodne.txt");
	int size[] = {5,10,20};
	for(int FUN = 0; FUN <3; FUN++)
	{	
		for(int SIZE : size)
		{
			//std::cout << SIZE << " " << FUN << std::endl;
			int signum = 1;
			...
			if(SIZE == 10 && (FUN == 0 || FUN == 2))
			{
				save_vector(plik_m,x,"");
				save_vector(plik_m,m,"");
				gsl_vector * m2 = druga_pochodna(FUN,x);
				save_vector(plik_m,x,"");
				save_vector(plik_m,m2,"");
				plik_m << std::endl;
			...
			for(int j = 0; j < M; j++)
			{
				...
				std::cout << j << " " << i << " " <<temp  << std::endl;
				if(j!= 0 && j%temp == 0 && i < SIZE-1)
					i++; // licznik przedziału
			}
			//save_vector(plik,s_1,("F: " + std::to_string(FUN) + " N: " + std::to_string(SIZE)).c_str());
			
			save_vector(plik,x_2,"");
			save_vector(plik,s_1,"");
		}
	}
	plik.close();
	plik_m.close();
...
gsl_vector *druga_pochodna(int nr, gsl_vector *x)
{
    ...
    {
        double temp = (fun(gsl_vector_get(x,i) - h,nr) - 2*fun(gsl_vector_get(x,i),nr) + fun(gsl_vector_get(x,i) + h,nr)) / (h*h);
