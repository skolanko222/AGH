...
void GUIMyFrame1::m_b_rotate_click( wxCommandEvent& event )
{
 // TO DO: Obrot o 30 stopni
    Img_Cpy=Img_Org.Rotate(30*M_PI/180, wxPoint(0., 0.), true, NULL);
    m_scrolledWindow->ClearBackground();
}
...
void GUIMyFrame1::m_b_prewitt_click( wxCommandEvent& event )
{
 // TO DO: Pionowa maska Prewitta
    ...
    for(int i=1; i<height-1; i++)
    {
        ...
        {
            int grad = Img_Org.GetRed(j - 1, i- 1) * -1
					+Img_Org.GetRed(j - 1, i) * -1
					+Img_Org.GetRed(j - 1, i + 1) * -1
					+Img_Org.GetRed(j + 1, i - 1) * 1
					+Img_Org.GetRed(j + 1, i) * 1
					+Img_Org.GetRed(j + 1, i + 1) * 1
					+Img_Org.GetRed(j,i) * 0
					+Img_Org.GetRed(j,i+1) * 0
					+Img_Org.GetRed(j,i-1) * 0;
            if(grad>255) grad=255;
            if(grad<0) grad=0;
            data[3 * (i * Img_Cpy.GetWidth() + j) + 0] = grad;
			data[3 * (i * Img_Cpy.GetWidth() + j) + 1] = grad;
			data[3 * (i * Img_Cpy.GetWidth() + j) + 2] = grad;
        }
    }
    m_scrolledWindow->ClearBackground();
}
...
void GUIMyFrame1::Contrast(int value)
{
 // TO DO: Zmiana kontrastu obrazu. value moze przyjmowac wartosci od -100 do 100
	...
	for(int i=0; i<size; i++)
    {
        int temp = value1*(data[i]-128)+128;
        if(temp<0) temp=0;
